...
export class ActionConfig implements ConfigInterface {
  ...
  constructor() {
    ...
    this.awsIamRoleArn =
      this.action === "stop" ? "" : core.getInput("aws_iam_role_arn");
    ...
    this.ec2InstanceType =
      this.action === "stop" ? "" : core.getInput("ec2_instance_type");
    this.ec2AmiId = this.action === "stop" ? "" : core.getInput("ec2_ami_id");
    this.ec2InstanceRootDiskSizeGB =
      this.action === "stop" ? "" : core.getInput("ec2_root_disk_size_gb");
    this.ec2InstanceRootDiskEbsClass =
      this.action === "stop" ? "" : core.getInput("ec2_root_disk_ebs_class");
    this.ec2InstanceIamRole =
      this.action === "stop" ? "" : core.getInput("ec2_instance_iam_role");
    this.ec2InstanceTags =
      this.action === "stop" ? "" : core.getInput("ec2_instance_tags");
    this.ec2InstanceTtl =
      this.action === "stop" ? "" : core.getInput("ec2_instance_ttl");
    this.ec2SubnetId =
      this.action === "stop" ? "" : core.getInput("ec2_subnet_id");
    this.ec2SecurityGroupId =
      this.action === "stop" ? "" : core.getInput("ec2_security_group_id");
    this.ec2SpotInstanceStrategy =
      this.action === "stop"
        ? ""
        : core.getInput("ec2_spot_instance_strategy").toLowerCase();
  }
}
